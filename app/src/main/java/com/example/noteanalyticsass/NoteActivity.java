...
public class NoteActivity extends AppCompatActivity implements NoteAdapter.ItemClickListener{
    ...
    private void GetAllNotes() {
        DocumentReference docRef = db.collection("Category").document(cid);

// Create a reference to the collection inside the document
        CollectionReference postsRef = docRef.collection("NoteA");
        postsRef.get()
                        ...
                        if (documentSnapshots.isEmpty()) {
                            ...
                            for (DocumentSnapshot documentSnapshot : documentSnapshots) {
                                if (documentSnapshot.exists()) {
                                    ...
                                    Intent i = new Intent(NoteActivity.this, DetailsActivity.class);

                                    Notes note = new Notes(id,name,cat);
    ...
    public void onItemClick(int position, String id) {
        Intent i = new Intent(NoteActivity.this, DetailsActivity.class);
        Notes note = new Notes(id, item.get(position).name.toString(),item.get(position).id);
        i.putExtra("Note", note);
        i.putExtra("cid",cid);
